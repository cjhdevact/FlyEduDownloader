;****************************************************************
;PROJECT: SmartEduDownloader
;FILE: SmartEduDownloader-Release-x64-InnoSetup.iss
...
[InstallDelete]
; Removed old files
Type: files; Name: "{app}\SmartEduDownloader.exe"
Type: files; Name: "{app}\Newtonsoft.Json.dll"
; Removed old links
Type: files; Name: "{group}\SmartEduDownloader 1.0.2.24091 (x64).lnk"
...
Source: ".\SmartEduDownloader\bin\x64\Release\Newtonsoft.Json.dll"; DestDir: "{app}"; Flags: ignoreversion
...
; *** 应用程序标题
SetupAppTitle=安装
SetupWindowTitle=安装 SmartEduDownloader
UninstallAppTitle=卸载
UninstallAppFullTitle=卸载 SmartEduDownloader

; *** 启动问题
PrivilegesRequiredOverrideTitle=选择安装模式
PrivilegesRequiredOverrideInstruction=选择安装模式
PrivilegesRequiredOverrideText1=SmartEduDownloader 可以为所有用户安装（需要管理员权限），或仅为您安装。
PrivilegesRequiredOverrideText2=SmartEduDownloader 只能为您安装，或为所有用户安装（需要管理员权限）。
...
BeveledLabel=SmartEduDownloader 安装程序
...
SelectDirDesc=您想将 SmartEduDownloader 安装在哪里？
SelectDirLabel3=安装程序将安装 SmartEduDownloader 到下列文件夹中。
...
; *** “准备安装”向导页
WizardReady=准备安装
ReadyLabel1=安装程序现在准备开始安装 SmartEduDownloader 到您的电脑中。
...
PreparingDesc=安装程序正在准备安装 SmartEduDownloader 到您的电脑中。
PreviousInstallNotCompleted=先前程序的安装/卸载未完成。您需要重新启动您的电脑才能完成安装。%n%n在重新启动电脑后，再运行 SmartEduDownloader 的安装程序。
...
InstallingLabel=安装程序正在安装 SmartEduDownloader 到您的电脑中，请稍等。

; *** “安装完成”向导页
FinishedHeadingLabel=SmartEduDownloader 安装完成
FinishedLabelNoIcons=安装程序已在您的电脑中安装了 SmartEduDownloader。
FinishedLabel=安装程序已在您的电脑中安装了 SmartEduDownloader。此应用程序可以通过选择安装的快捷方式运行。
ClickFinish=点击“完成”退出安装程序。
FinishedRestartLabel=要完成 SmartEduDownloader 的安装，安装程序必须重新启动您的电脑。您想要立即重新启动吗？
FinishedRestartMessage=要完成 SmartEduDownloader 的安装，安装程序必须重新启动您的电脑。%n%n您想要立即重新启动吗？
ShowReadmeCheck=是，我想查阅自述文件
YesRadio=是，立即重新启动电脑(&Y)
NoRadio=否，稍后重新启动电脑(&N)
;运行 SmartEduDownloader
RunEntryExec=运行 SmartEduDownloader
;阅读更新日志
...
UninstalledMost=SmartEduDownloader 卸载完成。%n%n有一些内容无法被删除。您可以手动删除它们。
UninstalledAndNeedsRestart=要完成 SmartEduDownloader 的卸载，您的电脑必须重新启动。%n%n您想立即重新启动电脑吗？
...
StatusUninstalling=正在卸载 SmartEduDownloader...

; *** Shutdown block reasons
ShutdownBlockReasonInstallingApp=正在安装 SmartEduDownloader。
ShutdownBlockReasonUninstallingApp=正在卸载 SmartEduDownloader。
...
UninstallProgram=卸载 SmartEduDownloader
LaunchProgram=运行 SmartEduDownloader
...
begin
	...
	while IsRunning<>0 do
		begin
		if Msgbox('安装程序发现 SmartEduDownloader 正在运行。'#13 #13 '请先关闭 SmartEduDownloader 按“是”继续安装，或者按“否”退出！', mbCriticalError, MB_YESNO) = idNO then
			begin
			Result :=false; //安装程序退出
			IsRunning :=0;
		end else begin
